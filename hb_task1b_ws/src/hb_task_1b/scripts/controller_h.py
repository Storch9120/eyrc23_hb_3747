...
class HBTask1BController(Node):
    ...
    def control_loop(self, x, y, theta):
            ...
            logger.info("goal received = "+str(x)+str(y)+str(theta))
            ...
            while(rclpy.ok()):
                ...
                self.get_logger().info("er_x="+ str(error_x)+ "_hb_" +str(self.hb_x))
                self.get_logger().info("er_y="+ str(error_y) +"_hb_" +str(self.hb_y))
                self.get_logger().info("er_z="+ str(error_theta)+"_hb_" +str(self.hb_theta))
                ...
                # if (abs(error_x) < 0.5 and abs(error_y) < 0.5 and abs(error_theta < 0.5)):
                #     self.get_logger().info("goal checked")
                ...
                #     self.index = 0
            #self.rate.sleep()
                if ((abs(error_x) and abs(error_y))==0):
                    logger.info("a goal has been reached")
                    return
                # self.rate.sleep() 
    ...
    while rclpy.ok():
        ...
        if ebot_controller.future.done():
            ...
            ####################################################
                ...
                logger.info("response structure="+str(response))
