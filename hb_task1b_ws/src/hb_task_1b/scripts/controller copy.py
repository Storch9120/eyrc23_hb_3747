...
class HBTask1BController(Node):
    ...
    def control_loop(self, index):
        ...
        while(rclpy.ok()):
            ...
            errox = self.x_g[index]-self.odom_x
            erroy = self.y_g[index]-self.odom_y
            errot = self.theta[index]-self.odom_t
            ...
            if (abs(errox) <0.5 and abs(erroy)<0.5 ) :
                time.sleep(3)
                logger.info("Goal has been reached. Index of Goal = "+ str(index))
                break
    ...
    while rclpy.ok():
        ...
        if ebot_controller.future.done():
            try:
                ...
                ebot_controller.control_loop(ebot_controller.index)
